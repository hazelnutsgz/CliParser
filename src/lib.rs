...
impl<'a, 'b> CliParser<'a, 'b> {
    ...
    pub fn add_rule_with_default(&mut self, name: &'a str, rule_type: RuleType, default_value: &'b str) -> Result<(), &'static str> {
        match rule_type {
            RuleType::Maybe => {
            ...
            RuleType::HasArg => {
                Err("Default value is meaningless!")
            },
        }
    }
    ...
    pub fn parse(& self, arg_list: &'a Vec<String>) -> Result<Matches, &'static str> {
        ...
        if arg_list.len() < 2 {
            ...
            while index < arg_list.len() {
                if arg_list[index].chars().nth(0).unwrap() != '+' { return Err("The input contains no flag"); }

                match self.rules.get(&arg_list[index][1..]) {
                    Some(rule) => {
                        match rule.rule_type {
                            RuleType::NoArg => {
                                opts.insert(&arg_list[index][1..], Opt {
                                    argument: None,
                                    rule_type: RuleType::NoArg,
                                });
                            },
                            RuleType::HasArg => {
                                if index + 1 >= arg_list.len() { return Err("Index out of bound"); }
                                if arg_list[index+1].chars().nth(0).unwrap() == '+' { return Err("Need argument!"); }
                    ...
                    _ => {
                        return Err("Cannot recognize rule")
                    }
                };
                index += 1;
            }
...
impl<'a, 'b> Matches<'a, 'b> {
    pub fn get_opt(&self, opt_str: &'b str) -> Result<Option<&'b str>, &'a str> {
        for (contact, &number) in self.opts.iter() { 
            println!("{}", contact);
        }
        match self.opts.get(opt_str) {
            Some(target_opt) => {
                match target_opt.rule_type {
                    RuleType::NoArg => Ok(None),
                    _ => Ok(target_opt.argument),
                }
            },
            _ => {
                Err("No argument found")
            }
        }
    }
}